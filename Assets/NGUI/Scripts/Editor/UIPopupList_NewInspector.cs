...
[CustomEditor(typeof(UIPopupList_New))]
public class UIPopupList_NewInspector : Editor
{
	UIPopupList_New mList;
	...
	public override void OnInspectorGUI ()
	{
		...
		mList = target as UIPopupList_New;
		...
		UIListItem uiListItem = EditorGUILayout.ObjectField("ListItem", mList.itemUI, typeof(UIListItem), true) as UIListItem;
		
		if (mList.itemUI != uiListItem)
		{
			RegisterUndo();
			mList.itemUI = uiListItem;
			if (uiListItem != null) 
				uiListItem.SetData(mList.selection);
		}
		...
		if (mList.atlas != null)
		{
			...
			*/
			...
			string itemPrefabPath = EditorGUILayout.TextField("ItemPrefab", mList.listItemPrefabPath);
			if (itemPrefabPath != mList.listItemPrefabPath)
			{
				mList.listItemPrefabPath = itemPrefabPath;
			}
			
			UIPopupList_New.Position pos = (UIPopupList_New.Position)EditorGUILayout.EnumPopup("Position", mList.position);
