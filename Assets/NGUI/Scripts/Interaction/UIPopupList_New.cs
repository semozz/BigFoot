...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/Popup List_New")]
public class UIPopupList_New : MonoBehaviour
{
	/// <summary>
	/// Current popup list. Only available during the OnSelectionChange event callback.
	/// </summary>

	static public UIPopupList_New current;
	...
	public delegate void OnSelectionChange (ListItemData item);
	...
	[HideInInspector][SerializeField] ListItemData mSelectedItem;
	UIPanel mPanel;
	GameObject mChild;
	UISprite mBackground;
	UISprite mHighlight;
	//UILabel mHighlightedLabel = null;
	UIListItem mHighlightedItem = null;
	//List<UILabel> mLabelList = new List<UILabel>();
	List<UIListItem> mItemList = new List<UIListItem>();
...
*/
	...
	void Start ()
	{
		...
		{
			RankListItemData data = new RankListItemData();
			data.rankTypeName = "Rank" + index;
			data.rankName = "RankName" + index;
	...
	void Highlight(UIListItem uiItem, bool instant)
	{
		if (mHighlight != null)
		{
			// Don't allow highlighting while the label is animating to its intended position
			TweenPosition tp = uiItem.GetComponent<TweenPosition>();
			if (tp != null && tp.enabled) return;

			//mHighlightedLabel = lbl;
			mHighlightedItem = uiItem;

			UIAtlas.Sprite sp = mHighlight.sprite;
			float offsetX = sp.inner.xMin - sp.outer.xMin;
			float offsetY = sp.inner.yMin - sp.outer.yMin;

			//Vector3 pos = uiItem.cachedTransform.localPosition + new Vector3(-offsetX, offsetY, 0f);
			Vector3 pos = uiItem.transform.localPosition + new Vector3(-offsetX, offsetY, 0f);
	...
	void Select(UIListItem uiListItem, bool instant)
	{
		//Highlight(lbl, instant);
		Highlight(uiListItem, instant);
		
		UIEventListener listener = uiListItem.gameObject.GetComponent<UIEventListener>();
		selection = listener.parameter as ListItemData;
	...
	void OnItemPress (GameObject go, bool isPressed) { if (isPressed) Select(go.GetComponent<UIListItem>(), true); }
	...
	void OnKey (KeyCode key)
	{
		if (enabled && NGUITools.GetActive(gameObject) && handleEvents)
		{
			...
			int index = mItemList.IndexOf(mHighlightedItem);
			
			if (key == KeyCode.UpArrow)
			{
				if (index > 0)
				{
					Select(mItemList[--index], false);
				}
			}
			else if (key == KeyCode.DownArrow)
			{
				if (index + 1 < mItemList.Count)
				{
					Select(mItemList[++index], false);
				}
			}
	...
	public string listItemPrefabPath = "";
	...
	void OnClick()
	{
		if (mChild == null && atlas != null && font != null && items.Count > 0)
		{
			...
			List<UIListItem> uiItems = new List<UIListItem>();
			
			// Run through all items and create labels for each one
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				ListItemData data = items[i];
				
				UIListItem uiListItem = ResourceManager.CreatePrefab<UIListItem>(listItemPrefabPath, mChild.transform, Vector3.zero);
				fontScale = uiListItem.itemHeight;
				
				uiListItem.transform.localPosition = new Vector3(bgPadding.x + padding.x, y, -0.01f);
				
				if (uiListItem == null)
					continue;
				
				uiListItem.SetData(data);
				
				uiItems.Add(uiListItem);
				
				y -= fontScale;
				y -= padding.y;
				//x = Mathf.Max(x, uiListItem.relativeSize.x * fontScale);

				// Add an event listener
				UIEventListener listener = UIEventListener.Get(uiListItem.gameObject);
				listener.onHover = OnItemHover;
				listener.onPress = OnItemPress;
				listener.parameter = data;

				// Move the selection here if this is the right label
				if (mSelectedItem == data) Highlight(uiListItem, true);

				// Add this label to the list
				//mLabelList.Add(lbl);
				mItemList.Add(uiListItem);
			}
			...
			for (int i = 0, imax = uiItems.Count; i < imax; ++i)
			{
				UILabel lbl = labels[i];
				BoxCollider bc = NGUITools.AddWidgetCollider(uiListItem.gameObject);
			...
			*/
			...
			if (isAnimated)
			{
				...
				//for (int i = 0, imax = uiItems.Count; i < imax; ++i) Animate(uiItems[i], placeAbove, bottom);
