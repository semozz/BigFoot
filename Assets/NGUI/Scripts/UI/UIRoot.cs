...
public class UIRoot : MonoBehaviour
{
	...
	private static float orgWidth = 1280.0f;
    private static float orgHeight = 720.0f;
    private static float ratio = 1.0f;
    private float orgVerRatio = orgHeight / orgWidth;
    private float orgHorRatio = orgWidth / orgHeight;
	
	private static float screenHeight = 0.0f;
    private static float screenWidth = 0.0f;
    private static bool useWidth = false;
    private static float screenSize = 2f;
	
	public static float ScreenWidth
	{
		get { return orgWidth * Screen.width / screenWidth; }
	}
	public static float ScreenHeight
	{
		get { return orgHeight * Screen.height / screenHeight; }
	}
	...
	void Update ()
	{
		if (mTrans != null)
		{
            ...
            if (Screen.width * orgVerRatio > Screen.height)
            {
                screenWidth = Screen.height * orgHorRatio;
				screenHeight = Screen.height;
                ratio = Screen.width / screenWidth;
                useWidth = false;
            }
            else
            {
				screenWidth = Screen.width;
                screenHeight = Screen.width * orgVerRatio;
                ratio = Screen.height / screenHeight;
                useWidth = true;
            }

            manualHeight = (int)orgHeight;

            if (useWidth)
                screenSize = 2f / orgHeight;
            else
                screenSize = 2f / (orgHeight * ratio);

            Vector3 ls = mTrans.localScale;

            if (!(Mathf.Abs(ls.x - screenSize) <= float.Epsilon) ||
                !(Mathf.Abs(ls.y - screenSize) <= float.Epsilon) ||
                !(Mathf.Abs(ls.z - screenSize) <= float.Epsilon))
            {
                mTrans.localScale = new Vector3(screenSize, screenSize, screenSize);
            }
		}
	}
