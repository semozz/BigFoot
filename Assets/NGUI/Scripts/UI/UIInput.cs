	...
	void OnSelect (bool isSelected)
	{
		...
		if (label != null && enabled && NGUITools.GetActive(gameObject))
		{
			if (isSelected)
			{
				...
				int caratIndex = mText.LastIndexOf(caratChar);
				if (caratIndex != -1)
					mText = mText.Remove(caratIndex, caratChar.Length);
			...
			else
			{
				...
				else
				{
					...
					if (this.maxChars > 0 && mText.Length > this.maxChars)
						mText = mText.Substring(0, this.maxChars);
